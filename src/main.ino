  ...
  if (client) {                             
    while (client.connected()) {            
      if (client.available()) {                                     // if there's client data
        respMsg = "Current position = " + String(currPos);   // HTTP Response Message
        // Read the first line of the request
        String req = client.readStringUntil('\r');
        processRequest(client,req);

        String s = userResponsePage(req);
...
void processRequest(WiFiClient client,String req){
  if (debugPrint ==true){
           Serial.println(req);
        }
...
String userResponsePage(String req){
  // Prepare the response
  String ui= "Something went wrong";
  if (req.indexOf("/api/") == -1){
    ui = "<!DOCTYPE html><html><head><meta name='viewport' content='initial-scale=1.0'><meta charset='utf-8'><style>#map {height: 100%;}html, body {height: 100%;margin: 25px;padding: 10px;font-family: Sans-Serif;} p{font-family:'Courier New', Sans-Serif;}</style>";
    ui += "<link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css\">";
    ui +="<script defer src=\"https://use.fontawesome.com/releases/v5.0.9/js/all.js\"></script></head>";        
    ui += "<body><h1>Stepper control through WiFi</h1>";
    ui += "<button type=\"button\" class=\"btn btn-primary btn-lg\" id=\"btn_down\"><i class=\"fas fa-arrow-alt-circle-down fa-2x\"></i></button>&nbsp;";
    ui += "<button type=\"button\" class=\"btn btn-primary btn-lg\" id=\"btn_up\"><i class=\"fas fa-arrow-alt-circle-up fa-2x\"></i></button>&nbsp;";
    ui += "<button type=\"button\" class=\"btn btn-dark btn-lg dropdown-toggle\" type=\"button\" id=\"ddMB\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><i class=\"fas fa-percent fa-2x\"></i>&nbsp;</button><div class=\"dropdown-menu\" aria-labelledby=\"ddMB\"><a class=\"dropdown-item\" id=\"btn-10\" href=\"#\">10%</a><a class=\"dropdown-item\"id=\"btn-20\" href=\"#\">20%</a><a class=\"dropdown-item\" id=\"btn-30\" href=\"#\">30%</a>";
    ui += "<a class=\"dropdown-item\" id=\"btn-40\" href=\"#\">40%</a><a class=\"dropdown-item\"id=\"btn-50\" href=\"#\">50%</a><a class=\"dropdown-item\" id=\"btn-60\" href=\"#\">60%</a>";
    ui += "<a class=\"dropdown-item\" id=\"btn-70\" href=\"#\">70%</a><a class=\"dropdown-item\"id=\"btn-80\" href=\"#\">80%</a><a class=\"dropdown-item\" id=\"btn-90\" href=\"#\">90%</a>";
    ui += "</div><br><br>";
    ui += "<div id=\"w\" class=\"alert alert-success\" role=\"alert\" style=\"display:none;\">One moment please.</div>";
    ui += "<div id=\"r\" class=\"alert alert-info\" role=\"alert\">" + respMsg + "</div>";
    ui += printUsage();
    ...
    ui +="function send(dowhat){$('#r').hide(); $('#w').show(); $.get({url:'/api/stepper/' + dowhat,success:function(data){$('#w').hide(); $('#r').html(data).show(); }}); } ";
    ui +="$('#btn_down').click(function(){send('movedown?" + String(maxSteps) + "');});";
    ui +="$('#btn_up').click(function(){send('moveup?" + String(maxSteps) + "');});";
    ui +="$('.dropdown-item').click(function(){ var per= $(this).attr('id').split('-');send('percent?'+per[1]);  });";
    ui +="});";
    ui += "</script></body></html>";
